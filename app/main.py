...
def register(payload: UserCreate):
    with Session(db.engine) as session:
        ...
        user = crud.create_user(
            session, payload.username, payload.email, hashed, payload.role
        )
...
def create_test2(payload: TestCreate, current_user=Depends(teacher_required)):
    with Session(db.engine) as session:
        t = crud.create_test(
            session, payload.title, current_user.id, payload.description
        )
        return t
...
def add_question(
    test_id: int, payload: QuestionCreate, current_user=Depends(teacher_required)
):
    with Session(db.engine) as session:
...
def submit(payload: SubmissionCreate, current_user=Depends(student_required)):
    with Session(db.engine) as session:
        ...
        questions = session.exec(
            models.Question.select().where(models.Question.test_id == test.id)
        ).all()
        ...
        for item in payload.answers:
            ...
            if qid not in q_map:
                raise HTTPException(
                    status_code=400, detail=f"question {qid} does not exist in test"
                )
...
def analytics(test_id: int, current_user=Depends(teacher_required)):
    with Session(db.engine) as session:
        ...
        subs = session.exec(
            models.Submission.select().where(models.Submission.test_id == test_id)
        ).all()
